...
async function loadPositions(grid) {
  try {
    ...
    grid.innerHTML = positions.map(p => {
      ...
      const borderGlow = pnl >= 0 ? 'rgba(0, 200, 5,0.22)' : 'rgba(255,77,94,0.22)';
...
async function loadTrades(tbody) {
  try {
    ...
    tbody.innerHTML = trades.map(t => {
      ...
      // Perp fills settle on Arbitrum (Ostium) — link Arbiscan
      const txLink = t.signature && String(t.signature).startsWith('0x') && String(t.signature).length === 66
        ? `<a href="https://arbiscan.io/tx/${t.signature}" target="_blank" rel="noopener" style="color:var(--accent);text-decoration:none;font-size:0.65rem;">view</a>`
        : '-';

      return `<tr style="border-bottom:1px solid rgba(255,255,255,0.04);transition:background 0.2s;" onmouseover="this.style.background='rgba(0, 200, 5,0.05)'" onmouseout="this.style.background='transparent'">
        <td style="padding:10px 8px;color:var(--text-primary);">${t.position || '-'}</td>
        <td style="padding:10px 8px;color:${actionColor};">${t.action}</td>
        <td style="padding:10px 8px;text-align:right;color:${depositColor};">$${depositStr}</td>
        <td style="padding:10px 8px;text-align:right;color:${pnlColor};">${pnlStr !== '-' ? '$' + pnlStr : '-'}</td>
