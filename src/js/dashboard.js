...
import { loadModalChart } from './charts.js';
...
async function fetchData() {
  try {
    ...
    return tokens.map(t => {
      ...
      return {
        address,
        token: t.name || t.symbol || t.token || address?.slice(0, 8) || 'UNKNOWN',
        symbol: t.symbol || '',
        underlying: t.underlying || t.linkedTo || '--',
        side: (t.side || pos.side || 'long').toLowerCase(),
        leverage: t.leverage || pos.leverage || 50,
        strategy: t.strategy || 'degen',
        entry: pos.entry || 0,
        sizeUsd: pos.sizeUsd || 0,
        collateralUsd: pos.collateralUsd || 0,
        deployedUsd: pos.deployedUsd || 0,
        pnl: pos.pnl || 0,
        pnlCurrency: pos.pnlCurrency || 'USD',
...
function renderTable() {
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="7" style="text-align:center;padding:60px 20px;">
          ...
          <div style="font-size:0.8rem;color:var(--text-tertiary);margin-bottom:20px;">Be the first to launch a Fill derivative.</div>
  ...
  tbody.innerHTML = data.map(row => {
    ...
    // Status pill for non-active positions
    const hasFees = !hasPosition && row.pnl > 0;
    const pillClass = row.statusText?.includes('Liquidated') || row.statusText?.includes('re-entry')
      ? 'pill-alert' : 'pill-collecting';
    const statusPill = `<span class="status-pill ${pillClass}">${row.statusText || 'Collecting fees'}</span>`;
    const fmtPnl = (v) => row.pnlCurrency === 'ETH' ? `${v.toFixed(4)} ETH` : `$${Math.abs(v).toFixed(2)}`;
    const statusCell = hasPosition
      ? `<td style="color:${pnlColor};font-family:var(--font-mono);">
          ${pnlSign}${fmtPnl(row.pnl)}
        </td>
      ...
      : hasFees
        ? `<td style="color:var(--green, #00ff88);font-family:var(--font-mono);">
          ...
          <td>${statusPill}</td>`
        : `<td colspan="2">${statusPill}</td>`;

    return `
    <tr class="dashboard-row" data-token="${row.address}" style="cursor:pointer;">
      <td>
  ...
  tbody.querySelectorAll('.dashboard-row').forEach(row => {
    row.addEventListener('click', () => {
      const address = row.getAttribute('data-token');
      const tokenData = currentData.find(d => d.address === address);
...
function openTokenModal(token) {
  ...
  const strategyText = document.getElementById('modal-strategy-text');
  if (strategyText) strategyText.textContent = (token.strategy || 'degen').toUpperCase() + ' MODE';
  ...
  if (deployedEl) deployedEl.textContent = token.deployedUsd > 0 ? `$${token.deployedUsd.toFixed(2)} USDC` : '—';
  ...
  if (mintEl) mintEl.textContent = token.address;
  ...
  fetchBuybackStats(token.address);

  // Shareable deep link (#token/0x…)
  if (location.hash !== `#token/${token.address}`) {
    history.replaceState(null, '', `#token/${token.address}`);
  }
  ...
  requestAnimationFrame(() => {
    ...
    // Chart canvas needs layout to have happened before it can size itself
    const market = token.market && token.market !== '--' ? token.market : 'AAPL';
    loadModalChart(market, token.address);
  });
}

/** Open a token's page directly from a #token/0x… link. */
function openTokenFromHash() {
  const m = location.hash.match(/^#token\/(0x[0-9a-fA-F]{40})$/);
  if (!m) return;
  const token = currentData.find(d => d.address?.toLowerCase() === m[1].toLowerCase());
...
async function fetchBuybackStats(mint) {
  ...
  const fillSolEl = document.getElementById('modal-fill-buyback-sol');
  const fillBurnEl = document.getElementById('modal-fill-tokens-burned');

  // Reset
  if (srcSolEl) srcSolEl.textContent = '--';
  if (srcBurnEl) srcBurnEl.textContent = '--';
  if (fillSolEl) fillSolEl.textContent = '--';
  if (fillBurnEl) fillBurnEl.textContent = '--';
  ...
  try {
    ...
    let fillSol = 0, fillTokens = 0;

    for (const b of buybacks) {
      if (b.type === 'source-buyback-burn') {
        sourceSol += b.amountEth || 0;
        sourceTokens += b.tokensBurned || 0;
      } else {
        fillSol += b.amountEth || 0;
        fillTokens += b.tokensBurned || 0;
      }
    }

    if (srcSolEl) srcSolEl.textContent = sourceSol > 0 ? `${sourceSol.toFixed(4)} ETH` : '0 ETH';
    if (srcBurnEl) srcBurnEl.textContent = sourceTokens > 0 ? formatNumber(sourceTokens) : '0';
    if (fillSolEl) fillSolEl.textContent = fillSol > 0 ? `${fillSol.toFixed(4)} ETH` : '0 ETH';
    if (fillBurnEl) fillBurnEl.textContent = fillTokens > 0 ? formatNumber(fillTokens) : '0';
  } catch {
...
async function loadActivityFeed(container) {
  try {
      ...
      for (const run of runs.slice(-30)) {
        if (run.feesClaimed > 0) {
          events.push({
            ...
            label: 'Fee Claimed',
            detail: `${(run.feesClaimed || 0).toFixed(6)} ETH`,
            txSig: run.txHash,
            color: 'var(--accent)',
          });
        }
        if (run.txHash && run.positionAmount > 0) {
          events.push({
            type: 'position',
            time: run.timestamp || run.createdAt,
            label: 'Position Funded',
            detail: `${(run.positionAmount || 0).toFixed(6)} ETH deployed`,
            txSig: run.txHash,
    ...
    if (buybacksRes && buybacksRes.ok) {
      ...
      for (const b of buybacks) {
        ...
        events.push({
          ...
          label: isSource ? 'Source Buyback & Burn' : 'FILL Buyback & Burn',
          detail: `${(b.amountEth || 0).toFixed(4)} ETH / ${formatNumber(b.tokensBurned || 0)} tokens burned`,
          txSig: b.swapTxHash,
          burnSig: b.burnTxHash,
    ...
    container.innerHTML = events.slice(0, 25).map(e => {
      ...
      const explorerLink = e.txSig
        ? `<a href="https://robinhoodchain.blockscout.com/tx/${e.txSig}" target="_blank" style="font-family:var(--font-mono);font-size:0.6rem;color:var(--text-muted);text-decoration:none;margin-left:8px;">[view tx]</a>`
        : '';
      const burnLink = e.burnSig
        ? `<a href="https://robinhoodchain.blockscout.com/tx/${e.burnSig}" target="_blank" style="font-family:var(--font-mono);font-size:0.6rem;color:var(--text-muted);text-decoration:none;margin-left:4px;">[burn tx]</a>`
        : '';
      ...
      return `
        <div style="display:flex;align-items:center;gap:12px;padding:10px 14px;border:1px solid var(--border);border-radius:3px;font-size:0.75rem;">
          ...
          <div style="flex:1;">
            <div style="font-family:var(--font-mono);color:var(--text-primary);">${e.label}${explorerLink}${burnLink}</div>
